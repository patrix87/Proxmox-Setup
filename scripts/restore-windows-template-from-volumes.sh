...
iso=/var/lib/vz/template/iso/WinServer2025_English_x64.iso
...
qm set $id --tpmstate0 ${storage},version=v2.0
# Create an EFI disk with raw format and pre-enrolled keys
qm set $id --efidisk0 ${storage},format=raw,efitype=4m,pre-enrolled-keys=1
# Create a SCSI disk with the specified size and raw format
qm set $id --scsi0 ${storage},format=raw
